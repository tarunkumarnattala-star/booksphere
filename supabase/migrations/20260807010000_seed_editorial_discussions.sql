...
If your schedule changes week to week, what actually held? And what quietly stopped working the moment your routine broke?', 'published'),
  ((select id from public.books where slug = 'thinking-fast-and-slow'), '11111111-1111-1111-1111-111111111111', 'Insight', 'insight', 'The most useful part of this book is the part that failed replication', 'Several priming studies in this book did not survive the replication crisis, and Kahneman said so publicly himself. That admission is more instructive than most of the findings.
...
Which raises the question worth discussing here: has knowing about anchoring ever actually changed a number you agreed to? Or did you only recognise it afterwards?', 'published'),
  ((select id from public.books where slug = 'deep-work'), '11111111-1111-1111-1111-111111111111', 'Question', 'question', 'Is deep work a skill, or a privilege dressed as a skill?', 'Newport argues that the ability to concentrate without distraction is becoming rare and therefore valuable. The economic logic is sound.
...
So: for those of you who do not control your own calendar, has any version of this worked? Or is the honest answer that the advice arrives about ten years into a career?', 'published'),
  ((select id from public.books where slug = 'the-lean-startup'), '11111111-1111-1111-1111-111111111111', 'Disagreement', 'disagreement', 'The MVP became a licence to ship things nobody wanted', 'Ries defined the minimum viable product as the smallest experiment that produces validated learning. In practice the term got used to justify shipping a thin product and calling the resulting silence data.
...
What is the passage you have returned to more than once, and did it work the second time?', 'published'),
  ((select id from public.books where slug = 'the-psychology-of-money'), '11111111-1111-1111-1111-111111111111', 'Insight', 'insight', 'Reasonable beats rational, and that is a harder standard than it sounds', 'Housel''s argument is that a financial plan you can actually keep during a downturn beats an optimal one you abandon. The claim is modest and widely agreed with in the abstract.
...
Has anyone here actually learned their risk tolerance in advance, rather than during a crash?', 'published'),
  ((select id from public.books where slug = 'never-split-the-difference'), '11111111-1111-1111-1111-111111111111', 'Disagreement', 'disagreement', 'Hostage negotiation tactics in an ordinary workplace can read as manipulation', 'Tactical empathy, mirroring, labelling and calibrated questions are genuinely effective. The techniques work, which is part of the problem.
...
For managers here who work distributed: what did you have to replace, and what did you never manage to replace?', 'published'),
  ((select id from public.books where slug = 'principles'), '11111111-1111-1111-1111-111111111111', 'Disagreement', 'disagreement', 'Radical transparency worked at Bridgewater. That is not evidence it travels.', 'Dalio''s system, believability-weighted decisions, recorded meetings, public ratings, produced extraordinary returns at one firm over decades. That is real evidence of something.
...
Has anyone tried importing a piece of this into a normal company? Which part transferred, and which part immediately broke?', 'published'),
  ((select id from public.books where slug = 'good-strategy-bad-strategy'), '11111111-1111-1111-1111-111111111111', 'Insight', 'insight', 'Most documents called strategy are goals with a deadline attached', 'Rumelt''s kernel is diagnosis, guiding policy, coherent action. The test he offers is simple and brutal: if your strategy does not identify a specific obstacle, it is not a strategy.
...
What decision are you carrying where you still cannot tell whether you got it right?', 'published'),
  ((select id from public.books where slug = 'crucial-conversations'), '11111111-1111-1111-1111-111111111111', 'Question', 'question', 'Does the method work when the power gap is large?', 'The framework, watch for safety, make it safe, state your path, is genuinely usable and unusually specific for a communication book.
...
Does the narrow version still predict anything? Where have you seen a genuine low-end entry that incumbents rationally ignored?', 'published'),
  ((select id from public.books where slug = 'how-to-win-friends-and-influence-people'), '11111111-1111-1111-1111-111111111111', 'Question', 'question', 'Does this book still work now that everyone has read it?', 'Written in 1936 and still selling. The advice, use someone''s name, be genuinely interested, avoid criticising directly, was less familiar then than now.
