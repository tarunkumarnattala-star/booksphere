...
drop constraint if exists knowledge_posts_body_check;

alter table public.knowledge_posts
drop constraint if exists knowledge_posts_launch_content_length;

alter table public.knowledge_posts
drop constraint if exists knowledge_posts_content_length;

alter table public.knowledge_posts
add constraint knowledge_posts_content_length check (
  char_length(trim(title)) between 4 and 180 and
  char_length(trim(body)) between 3 and 2000 and
