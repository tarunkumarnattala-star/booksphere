...
| Create account | **BROKEN — root cause found** | Confirmed by direct API probe on August 4: signup returns 500 because the profile trigger generates usernames longer than the 30-character cap in `profiles_launch_content_length`, which aborts the whole `auth.users` insert. Any email with a local part over 23 characters (including plus-tagged addresses) cannot create an account, and has been unable to since July 15. Fix is written in migration `20260805000000_fix_signup_username_length.sql` — **it must be applied in the SQL editor**. Separately: until a domain is verified in Resend, magic links deliver only to the Resend account owner's address. Google is hidden (provider disabled). |
