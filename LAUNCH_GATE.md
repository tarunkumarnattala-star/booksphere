...
- A discussion that no longer exists answers HTTP 200 with the not-found screen rather than a real 404. The same is true of `/post` and `/profile`: all three are dynamic and database-backed, so their params cannot be pre-validated the way book and genre slugs are in the proxy. It costs a little SEO on dead links and nothing else.
